...
export async function saveAuthState(state: string): Promise<void> {
  ...
  cookieStore.set(STATE_COOKIE_NAME, state, COOKIE_OPTIONS);
}
...
export async function saveTokens(
  ...
  const cookieStore = await cookies();
  cookieStore.set(ACCESS_TOKEN_COOKIE_NAME, access_token, COOKIE_OPTIONS);
  cookieStore.set(
  ...
  if (refresh_token) {
    cookieStore.set(REFRESH_TOKEN_COOKIE_NAME, refresh_token, COOKIE_OPTIONS);
  }
}

export async function saveUserProfile(user: SpotifyUser): Promise<void> {
  const cookieStore = await cookies();
  cookieStore.set(USER_COOKIE_NAME, JSON.stringify(user), COOKIE_OPTIONS);
}
